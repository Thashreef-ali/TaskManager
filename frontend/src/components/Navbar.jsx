...
function Navbar({ user = {}, onLogout }) {
  ...
  useEffect(()=>{
    const hanldleClickOutside = (event) =>{
      if(menuref.current  && !menuref.current.contains(event.target)){
        setmenuOpen(false)
      }
    }
    document.addEventListener('mousedown',hanldleClickOutside)
    return ()=> document.removeEventListener('mousedown',hanldleClickOutside)
  },[])
  return (
      ...
      <div className="flex items-center justify-between px-4 py-3 md:px-6 max-w-7xl mx-auto">
        ...
        >
          <div className="relative w-10 h-10 flex items-center justify-center rounded-xl bg-gradient-to-br from-fuchsia-500 via-purple-500 to-indigo-500 shadow-lg group-hover:shadow-purple-300/50 group-hover:scale-105 transition-all duration-300">
            ...
            <div className="absolute -bottom-1 -middle-1 w-3 h-3 rounded-full shadow-md animate-ping" />
          </div>
          <span className="text-2xl font-extrabold bg-gradient-to-r from-fuchsia-500 via-purple-500 to-indigo-500 text-transparent bg-clip-text">
            TaskManager
          </span>
        </div>
        <div className="flex items-center gap-4">
          <button
            className="p-2 text-gray-600 hover:text-purple-600 transition-colors duartion-300 hover:bg-purple-50 rounded-full"
          ...
          <div ref={menuref} className="relative">
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 rounded-full cursor-pointer hover:bg-purple-50 transition-colors duration-300
              ...
              <ul className=" absolute top-14 right-0 w-56 bg-white rounded-2xl shadow-xl border border-purple-100 z-50 overflow-hidden animate-fadeIn">
                ...
                <li className="p-2 ">
                  <button
                    ...
                    className=" w-full flex items-center  gap-2 rounded-lg px3 py-2 text-sm hover:bg-red-50 text-red-500"
                  >
