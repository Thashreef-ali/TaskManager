  ...
  const fetchTasks = useCallback(async ()=>{
    ...
    try {
      ...
      if(!token) throw new Error('No auth token found')
        const response = await axios.get('https://taskmanager-backend-v5cr.onrender.com/api/gp', {
  headers: { Authorization: `Bearer ${token}` }
});
    const data = response.data;  // <-- get actual data from response
          ...
          <div className='xl:col-span-1 space-y-4 sm:space-y-6'>
            <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100'>
              ...
              <div className='grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6'>
                <StatCard title='Total Tasks' value={stats.totalCount} icon={<Circle className='w-3.5 h-3.5 sm:h-4 sm:w-4 text-purple-500'/>}/>
                <StatCard title='Completed' value={stats.completedTasks} icon={<Circle className='w-3.5 h-3.5 sm:h-4 sm:w-4 text-green-500'/>}/>
                <StatCard title='Pending' value={stats.pendingCount} icon={<Circle className='w-3.5 h-3.5 sm:h-4 sm:w-4 text-fuchsia-500'/>}/>
            ...
            <div className='bg-white rounded-xl p-4 sm:p-5  shadow-sm border border-purple-100'>
              <h3 className='text-base sm:text-lg font-semibold mb-3 sm:mb-4 text-gray-800 flex items-center gap-3'>
                  <Clock className='w-4 h-4 sm:w-5 sm:h-5 text-purple-500 '/>
                  Recent Activity
              </h3>
              <div className='space-y-2 sm:space-y3 '>
                {tasks.slice(0,3).map((task)=>(
                  <div key={task._id || task.id} className='flex items-center justify-between p-2 sm:p-3 hover:bg-purple-50/50 rounded-lg transition-colors duration-200 border border-transparent  hover:purple-100'>
                    ...
                    <span className={`px-3 py-1 text-xs rounded-full shrink-0 ml-2 ${task.completed? 'bg-green-100 text-green-700':'bg-fuchsia-100 text-fuchsia-700'}`}>
